...
db.serialize(() => {
  ...
  // 1. 全部を一時番号に移動
  db.run('UPDATE cards SET number = ? WHERE id = ?', [temp1, 'red_eared_slider'], function(err) {
    if (err) {
      console.error('Step 1 error:', err);
      db.run('ROLLBACK');
      return;
    }
    console.log('Step 1: Mアカミミガメ → 一時番号');
    
    db.run('UPDATE cards SET number = ? WHERE id = ?', [temp2, 'coral'], function(err) {
      if (err) {
        console.error('Step 2 error:', err);
        db.run('ROLLBACK');
        return;
      }
      console.log('Step 2: サンゴ → 一時番号');
      
      db.run('UPDATE cards SET number = ? WHERE id = ?', [temp3, 'tulip'], function(err) {
        if (err) {
          console.error('Step 3 error:', err);
          db.run('ROLLBACK');
          return;
        }
        console.log('Step 3: チューリップ → 一時番号');
