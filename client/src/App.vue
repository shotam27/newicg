  ...
  methods: {
    initializeSocket() {
      ...
      this.socket.on("waiting-for-opponent", () => {
        ...
        this.addMessage("対戦相手を探してぁE��ぁE..", "info");
      });
      ...
      this.socket.on("gameEnd", (data) => {
        ...
        this.addMessage(`ゲーム終亁E ${data.winner || "引き刁E��"}`, "success");
      });
      ...
      this.socket.on("select-target", (data) => {
        ...
        console.log("pendingAbilityに設宁E", this.pendingAbility);
      ...
      this.socket.on("unimplemented-effect", (data) => {
        ...
        this.addMessage(`📝 琁E��: ${data.unimplementedInfo.reason}`, "info");
      });
    },
    ...
    updateGameState(state) {
      ...
      if (state.players && state.players[this.socket.id]) {
        ...
        if (opponentId) {
          const wasFirstMatch = !this.opponentName; // 初回マッチングかチェチE��
